...
export class MytradesComponent implements OnInit {
  ...
  // Check if the trade involves a suspended user or blocked item
isTradeSuspendedOrBlocked(trade: any): boolean {
  // Check if the user is suspended or item is blocked
  const userSuspended = trade.item1.userId.isSuspended || trade.item2.userId.isSuspended;
  const itemBlocked = trade.item1.isBlocked || trade.item2.isBlocked;

  if (userSuspended || itemBlocked) {
    return true;
  }
  return false;
}
