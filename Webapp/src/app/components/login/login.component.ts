...
//   //       localStorage.setItem('token', `Bearer ${response.token}`);  // Added 'Bearer ' prefix
//   //       localStorage.setItem('user', JSON.stringify(response.user));
  
//   //       // Redirect based on whether the user is an admin or not
//   //       if (response.user.isAdmin) {  // Check if the user is an admin
...
//   //       // Handle login failure (you can also display error messages to the user here)
//   //     }
//   //   );
//   // }
//   login() {
...
//         // Redirect based on whether the user is an admin or not
//         if (response.user.isAdmin) {  // Use response.user.isAdmin instead of response.isAdmin
//           this.router.navigate(['/admin/allitems']);  // Redirect to Admin All Items page
//         } else {
//           this.router.navigate(['/home']);  // Redirect to Home page for normal users
//         }
...
//       (error) => {
//         console.error('Login failed:', error);
//         // Handle login filure (e.g., display error message in UI)
...
export class LoginComponent {
  ...
  login() {
    ...
    this.authService.login(email!, password!).subscribe({
      next: (result: any) => {
        // If account is not activated, show an error
        if (!result.user.isActivated) {
          this.errorMessage = 'Please activate your account. Check your email for the activation link.';
          return;
        }
        ...
        // If the account is activated and not suspended, store the token and user info
        localStorage.setItem('token', `Bearer ${result.token}`);  // Added 'Bearer ' prefix for the token
        localStorage.setItem('user', JSON.stringify(result.user));
      ...
      error: (error) => {
        ...
        if (error.error.error) {
          this.errorMessage = error.error.error;  // Show error message from backend
        } else {
          this.errorMessage = 'Invalid email or password. Please try again.';  // Fallback error message
        }
