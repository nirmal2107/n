...
router.get('/trades', verifyToken, async (req, res) => {
  ...
  try {
    ...
    .populate('item1', 'title size condition preferences image username isBlocked')
      .populate('item2', 'title size condition preferences image username isBlocked')
      .populate('userId2', 'name isSuspended')
    ...
    .populate('item1', 'title size condition preferences image username isBlocked')
    .populate('item2', 'title size condition preferences image username isBlocked')
    .populate('userId1', 'name isSuspended')
    .skip((page - 1) * pageSize)
    .limit(pageSize);
...
router.put('/trades/:tradeId/status', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!trade.userId2.equals(objectId)) { // Compare ObjectId using .equals()
      return res
        .status(403)
