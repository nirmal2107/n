...
@Component({
  ...
  imports: [ReactiveFormsModule, CommonModule,FormsModule], // Import ReactiveFormsModule here
...
export class TradeItemsFormComponent {/*
  ...
  onSubmit(): void {
    ...
    constructor(private tradeItemService:AllItemsService, private authservice: AuthService, private router :Router ) {}
    ...
    onSubmit(): void {
      ...
      const preferencesArray = this.tradeItem.preferences.split(',').map(pref => pref.trim());
